...
// =======================
// Create room using generated token
// =======================
app.post("/api/create-room", async (req, res) => {
  ...
  try {
    // 1️⃣ Generate a temporary token (no room yet) to call the API
    const tempToken = generateToken({}); // just apikey + version

    // 2️⃣ Call VideoSDK API to create a new room
    ...
    // 3️⃣ Generate host token for this room (to be used for WebRTC / SDP)
    const hostToken = generateToken({ userId, roomId, role: "host" });

    // ✅ Return room info and host token
