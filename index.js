...
import fetch from "node-fetch"; // needed to call VideoSDK API
...
// =======================
// 2️⃣ Create room using the server token
// =======================
app.post("/api/create-room", async (req, res) => {
  const { userId, serverToken } = req.body;

  if (!userId) return res.status(400).json({ success: false, error: "Missing userId" });
  if (!serverToken) return res.status(400).json({ success: false, error: "Missing serverToken" });
  ...
  try {
    ...
    const roomResp = await fetch("https://api.videosdk.live/v2/rooms", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${serverToken}`, // ✅ Use the server token
    ...
    res.json({ success: true, userId, roomId: roomData.roomId });
