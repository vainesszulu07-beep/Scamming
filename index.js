...
// =======================
// VIDEOSDK KEYS (your token logic keys)
// =======================
...
// =======================
// 🔑 GENERATE RAW TOKEN (your token logic)
// =======================
app.post("/api/token", (req, res) => {
  try {
    const { userId, roomId, role } = req.body;

    // Basic payload for token
    let payload = { apikey: VIDEOSDK_API_KEY, version: 2 };

    // If roomId & userId provided → host/viewer token
    if (roomId && userId) {
      payload = {
...
app.post("/api/create-room", async (req, res) => {
  ...
  try {
    ...
    // Host token using your token logic
...
app.post("/api/get-viewer-token", async (req, res) => {
  ...
  if (!roomId || !viewerId || !viewerName) return res.status(400).json({ error: "Missing fields" });
...
// TEST ROUTE
// =======================
app.get("/", (req, res) => res.send("VideoSDK backend + token server running ✅"));
