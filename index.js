...
// =======================
// Generate server token (authorize server to call VideoSDK API)
// =======================
function generateServerToken() {
  return jwt.sign(
    { apikey: API_KEY, version: 2 },
    API_SECRET,
...
// =======================
// Generate host token (frontend uses this to join as host)
// =======================
...
app.post("/api/create-room", async (req, res) => {
  ...
  try {
    ...
    const roomResponse = await fetch("https://api.videosdk.live/v2/rooms", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${serverToken}`, // MUST include Bearer
