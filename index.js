...
function generateToken({ userId, roomId, role }) {
  ...
  // Only include room info if we already have a roomId
  if (roomId && userId) {
    payload = {
      apikey: API_KEY,
      roomId,
...
app.post("/api/create-room", async (req, res) => {
  ...
  try {
    // 1️⃣ Temporary token for room creation (no roomId yet)
    const tempToken = generateToken({});

    // 2️⃣ Call VideoSDK API to create room
    ...
    const roomId = roomData.roomId; // ✅ official roomId from VideoSDK

    // 3️⃣ Generate host token for this room
    const hostToken = generateToken({ userId, roomId, role: "host" });

    // 4️⃣ Return all necessary info
    res.json({
      success: true,
      userId,
      roomId,
